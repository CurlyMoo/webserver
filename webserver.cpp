...
#else
      if(client->client.write_P((unsigned char *)PSTR("\r\n"), 2) > 0) {
        client->lastseen = millis();
      }
#endif
    }
...
#else
        if(client->client.write_P((unsigned char *)PSTR("0\r\n\r\n"), 5) > 0) {
          client->lastseen = millis();
        }
#endif
        i += 5;
          ...
          if(http_parse_request(client, &rbuffer, &size) == 0) {
            if(client->method == 1) {
               ...
               if(client->reqtype == 0) {
                ...
                client->substep = 0;
                if(http_parse_multipart_body(client, (unsigned char *)&rbuffer[x], size) == -1) {
                  client->step = WEBSERVER_CLIENT_CLOSE;
                }
              }
